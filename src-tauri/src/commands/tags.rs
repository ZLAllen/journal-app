...
pub fn assign_tag_to_entry(db: &DbConnection, entry_id: String, tag_id: String) -> Result<()> {
    let conn = db.conn();
...
pub fn remove_tag_from_entry(db: &DbConnection, entry_id: String, tag_id: String) -> Result<()> {
    let conn = db.conn();
...
pub fn get_all_tags(db: &DbConnection) -> Result<Vec<Tag>> {
    ...
    let tags = stmt.query_map([], |row| Ok(Tag::from_row(row.get(0)?, row.get(1)?)))?;
...
pub fn get_entries_with_tag(db: &DbConnection, tag_id: String) -> Result<Vec<String>> {
    ...
    let entry_ids = stmt.query_map(params![&tag_id], |row| row.get(0))?;
...
mod tests {
    ...
    fn test_delete_tag() {
        let db = setup_db();
        let tag = create_tag(&db, "temporary".to_string()).expect("Failed to create tag");
    ...
    fn test_assign_tag_to_entry() {
        ...
        let tags = get_tags_for_entry(&db, entry.id).expect("Failed to get tags");
    ...
    fn test_get_entries_with_tag() {
        ...
        let entries_with_tag =
            get_entries_with_tag(&db, tag.id).expect("Failed to get entries with tag");
